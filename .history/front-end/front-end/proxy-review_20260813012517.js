...

{
  ...
  console.log(`正常赋值后，player.goals = ${player.goals}`); // 201

  // ⚠️ 2. 绕过代理直接修改：不会触发校验（这是 Proxy 的特性）
  player.goals = 999; 
  console.log(`绕过代理修改后，player.goals = ${player.goals}`); // 999
  ...
  // ❌ 4. 触发 TypeError：传入字符串
  try {
    playerProxy.goals = "帽子戏法"; 
  } catch (error) {
    console.log(`捕获错误 [${error.name}]: ${error.message}`); 
  }

  // ❌ 5. 触发 TypeError：传入 NaN
  try {
    playerProxy.goals = NaN; 
  } catch (error) {
    console.log(`捕获错误 [${error.name}]: ${error.message}`); 
  }

  // ✅ 6. 修改未被拦截的属性：正常生效
  playerProxy.name = "埃尔林·哈兰德";
  console.log(`修改名字后，player.name = ${player.name}`); 