// resolve 和 reject 看的是位置，不是变量名
const promise = new Promise((resolve, reject) => {
  resolve("进球有效")
  reject("进球无效")
})
promise.then(result => console.log(result))


// then() 的核心：每次都返回一个新 Promise
// 1.then()返回普通值，值为该返回值
const p1 = Promise.resolve(0)
const p2 = p1.then(value => value + 1)
console.log(p1 === p2); //false


// 2.then()没有返回值，undefined
// 链式调用，如果then里面没有返回值，就默认返回undefined
Promise.resolve(0)
  .then(scores => scores + 1)
  .then(scores => { scores + 1 })//undefined，没有返回值
  .then(scores => console.log(scores))


// 3.then()返回另一个Promise
// 等待并采用该Promise的结果
Promise.resolve("梅西拿球")
  .then((result) => {
    console.log(result);
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve("梅西完成射门")
      }, 1000)
    })
  })
  .then(result => console.log(result))


// 4.then()抛出异常
// 新 Promise 失败，错误沿链向后寻找失败处理器：
Promise.resolve("开始进攻")
  .then((result) => {
    console.log(result);
    throw new Error("球被抢断")
  })
  .then(() => console.log("完成射门"))
  .catch(error => console.log(error.message)) //球被抢断


// catch()：错误传播与错误恢复
// catch(onRejected)可以理解为then(undefined,onRejected)
// 错误会跳过中间没有错误处理器的步骤
Promise.resolve()
  .then(() => {
    throw new Error("点球罚丢")
  })
  .then(() => "庆祝进球")
  .then(() => "继续庆祝")
  .catch(error => {
    console.log(error.message)
    return "调整心态" //返回了一个正常值，错误已被处理，Promise链恢复为成功状态
  })
  .then((result) => {
    console.log(result);
  })


// finally()：做收尾，不负责改变数据
Promise.resolve("比赛获胜")
  .finally(() => {
    console.log("关闭直播画面");
  })
  .then(console.log); // 比赛获胜

// 如果finally抛出或返回一个失败的Promise，链会保持失败
Promise.resolve("比赛获胜")
  .finally(() => {
    throw new Error("保存比赛数据失败")
  })
  .catch(error => console.log(error.message))


// 快速得到一个成功的Promise,Promise.resolve()
// 如果传入的本身是 Promise，它会采用这个 Promise 的最终状态，而不是简单包成普通值。
Promise.resolve("凯恩获得欧洲金靴奖！").then(console.log)
// 快速得到一个失败的Promise,Promise.reject()
Promise.reject(new Error("进球越位"))
  .catch(error => console.log(error.message))


// Promise.all()，全部成功才成功，返回值顺序和输入顺序一致，而不是与完成顺序一致
const slow = new Promise(resolve => {
  setTimeout(() => resolve('慢任务'), 1000)
})
const fast = Promise.resolve("快任务")
// 任何一个失败，all() 返回的 Promise 就会失败。但其他已经启动的任务通常仍会继续运行，Promise.all() 不会自动取消它们。
Promise.all([slow, fast]).then(console.log)


//  Promise.race(iterable)
// 第一个确定状态的 Promise 决定最终结果，无论第一个是成功还是失败。
Promise.race([
  new Promise(resolve => setTimeout(() => resolve("C罗到达终点"), 2000)),
  new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error("姆巴佩受伤")), 1000);
  })
]).catch(error => console.log(error.message));

const request = new Promise(resolve => {
  setTimeout(() => {
    resolve("比赛数据")
  }, 3000);
})
const timeout = new Promise((resolve, reject) => {
  setTimeout(() => {
    reject("请求超时")
  }, 2000);
})

Promise.race([request, timeout])
  .then(result => console.log(result))
  .catch(error => console.log(error))


// Promise.allSettled(iterable)
// 等待全部 Promise 确定状态，从不因为其中一个失败而提前结束：
Promise.allSettled([
  Promise.resolve("梅西进球"),
  Promise.reject("姆巴佩受伤")
]).then(result => console.log(result))
// 结果：
// [
//   { status: 'fulfilled', value: '梅西进球' },
//   { status: 'rejected', reason: '姆巴佩受伤' }
// ]


// 在函数前添加async，意味着这个函数一定返回Promise
async function getScore() {
  return 3
}
// const score = await getScore()
// console.log(score);
// 可以理解为：
function getScore() {
  return Promise.resolve(3)
}
const result = getScore()
console.log(Object.getPrototypeOf(result) === Promise.prototype);
result.then(score => console.log(score))


// 如果 async 函数抛出错误，调用者得到一个rejected Promise：
async function takePenalty() {
  throw new Error("点球罚丢")
}

takePenalty().catch(error => {
  console.log(error.message);
})


// await接收一个值，并等待它转换得到的Promise确定状态。
// Promise成功：await取出成功值。
async function example() {
  const result = await Promise.resolve("梅西进球")
  console.log(result);//梅西进球
}
example()


// Promise 失败：await 在当前位置抛出失败原因。
async function example() {
  try {
    const result = await Promise.reject(new Error("进球越位"))
    console.log(result); // 不执行
  } catch (error) {
    console.log(error.message);
  }
}
example()

// await后面不是Promise也可以
async function example() {
  const score = await 3
  console.log(score);
}
//近似理解为先执行
Promise.resolve(3)


// await 暂停的到底是谁
// await 只暂停当前 async 函数后面的代码，不会阻塞整个 JavaScript 程序。
function waitForMatch() {
  return new Promise(resolve => {
    setTimeout(() => resolve("比赛结束"), 1000);
  });
}

async function watchMatch() {
  console.log("1. 开始观看");

  const result = await waitForMatch();

  console.log("3.", result);
}

watchMatch();
console.log("2. 去做其他事情");


//  await 与链式传值
// Promise写法
Promise.resolve(0)
  .then(score => score + 1)
  .then(score => score + 1)
  .then(score => console.log(score))

//async/await写法
// 在 Promise 链中，需要使用 return 将值交给下一个 then()；
// 在 async 函数中，可以用普通变量保存 await 取得的值。
async function calculateScore() {
  let score = await Promise.resolve(0)
  score = score + 1
  score = score + 1
  console.log(score);
  return score
}

// try/catch/finally 对应什么
getPlayer()
  .then(player => getMatches(player.id))
  .then(matches => console.log(matches))
  .catch(error => console.error(error))
  .finally(() => console.log("查询结束"));

// async/await：
async function showMatches() {
  try {
    const player = await getPlayer();
    const matches = await getMatches(player.id);
    console.log(matches);
  } catch (error) {
    console.error(error);
  } finally {
    console.log("查询结束");
  }
}

// 串行和并发的区分
// 串行执行
async function trainSerially() {
  const messi = await trainMessi(); // 等 1 秒
  const ronaldo = await trainRonaldo(); // 再等 1 秒
  return [messi, ronaldo];
}
// 并发执行
async function trainTogether() {
  const messiTask = trainMessi();
  const ronaldoTask = trainRonaldo();

  const [messi, ronaldo] = await Promise.all([
    messiTask,
    ronaldoTask
  ]);

  return [messi, ronaldo];
}


// 调用 async 函数
//   ↓
// 同步执行，直到遇到 await
//   ↓
// 计算 await 右边的表达式
//   ↓
// 把结果按 Promise 处理
//   ├── fulfilled：取得 value
//   └── rejected：在 await 位置抛出 reason
//   ↓
// 当前 async 函数暂时退出，并向调用者返回 Promise
//   ↓
// Promise 确定状态后，通过微任务恢复 await 后面的代码
//   ↓
// async 函数 return → 返回的 Promise fulfilled
// async 函数 throw  → 返回的 Promise rejected
